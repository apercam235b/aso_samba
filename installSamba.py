...
def instalar_paquetes():
	...
	paquetes = ["samba", "smbclient", "krb5-config", "krb5-user", "winbind", "realmd", "libnss-winbind", "libpam-winbind"]
...
def cambiar_nombre(nombre_dominio, nombre_host):
	...
	for i in range(len(lineas)):
		if lineas[i].startswith(f"127.0.0.1	localhost"):
			lineas[i + 1] = f"172.0.1.1	{nombre_host}\n"
			lineas[i + 2] = f"{ip}	{nombre_completo + ' ' + nombre_host}\n"
...
def modificar_kerberos(dominio):
	...
	dominio_mayus = dominio.upper()
	n_equipo_dominio = "dig SRV _ldap._tcp.{dominio} | grep .{dominio} | tail -1 | cut -d'.' -f1-3"
