...
def instalar_paquetes():
	actualizacion = "sudo apt update --quiet 2>&1 > archivo.log"
...
def cambiar_nombre(nombre_dominio, nombre_host):
	...
	ip_divida = ip.split("/")
	#ENCUENTRA EL NOMBRE DE LA INTERFAZ
	...
	#LEE EL FICHERO Y REEMPLAZA LAS LINEAS.
	...
	#NO REINICIO EL SERVICIO POR QUE DA ERRORES CON LAS DNS
...
def reiniciar_servicios():
	servicios = ['nmbd', 'smbd', 'networking']
	for servicio in servicios:
		comando = ['sudo', 'systemctl', 'restart', servicio]
		try:
			subprocess.run(comando, check=True)
			print(f"Servicio {servicio} reiniciado correctamente.")
		except subprocess.CalledProcessError as e:
			print(f"Error al reiniciar el servicio {servicio}. Detalles: {e}")

def unirse_a_ad(dominio, usuario, contrasena):
	comando = ['sudo', 'net', 'ads', 'join', '-U', f'{usuario}%{contrasena}', dominio]
	try:
		resultado = subprocess.run(comando, check=True, capture_output=True, text=True)
		print("Éxito al unirse al dominio:")
		print(resultado.stdout)
	except subprocess.CalledProcessError as e:
		print(f"Error al intentar unirse al dominio. Detalles: {e}")
...
cambiar_dns(dns)
	#modificar los ficheros samba
modificar_samba(domain, nombre_equipo)
	#modificar la configuracion de kerberos
modificar_kerberos(domain)
	#reinicar los servicios
reiniciar_servicios()
	#unirnos al dominio
unirse_a_ad(domain, admin_user, admin_pass)
